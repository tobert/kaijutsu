...
shell easy: a long-lived `session_id` + a read loop, not a long-lived kaish.
...
- **True contextless materialization.** A new variant that skips the `/v/docs`/
  ...
  `kj attach`, `kj workspace`, `kj rc`…). More honest to "contextless," but it's a
  new code path and the VFS is bare until the user attaches.
...
## VFS shape — second open decision

Today the VFS shows **one** context's docs at `/v/docs`. Two ways to "work with
contexts through the VFS":

- **Attach-and-reflow** (reuses everything): one context visible at a time; `kj attach
  <ctx>` flips `SessionContextMap` and the next prompt re-points `/v/docs`. This is the
  loop above. No new VFS work.
- **Multi-context browse tree** (new work): expose all contexts at once, e.g.
  `/v/contexts/<id>/docs/…`, so the user can `ls` and `cd` across contexts without
  attaching. Genuinely new VFS code; defer past v1.

Ship attach-and-reflow first.
