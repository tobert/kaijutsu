...
|---|---|---|
| `02-kernel-anatomy.svg` | persistence & the journal lifecycle (write → oplog → snapshot → cold-start replay) | [README](../README.md#the-kernel-the-instruments-body), [kernel](../kernel.md) |
...
`06-crate-deps.svg` drew a crate now folded into `kaijutsu-kernel`, with
three dependency arrows into it that no longer exist. Both errors are structural — boxes and edges,
