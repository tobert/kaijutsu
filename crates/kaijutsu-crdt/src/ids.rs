//! Typed identifiers for kernels and contexts.
//!
//! Both `KernelId` and `ContextId` wrap UUIDv7 (time-ordered, globally unique).
//! They're opaque on the wire (16 bytes / `Data` in Cap'n Proto) and display
//! as standard UUID text for logging. The `short()` form (first 8 hex chars)
//! is for human-facing UI — never used as a lookup key.
...
mod tests {
    ...
    fn test_ordering_is_time_ordered() {
        // UUIDv7 is time-ordered, so sequential creation should be ordered
    ...
    fn test_resolve_hex_prefix() {
        ...
        // This may match both if they share a prefix, but with UUIDv7 it's unlikely
        let entries = vec![(a, None), (b, None)];
        let result = resolve_context_prefix(entries.into_iter(), prefix);
        // Either succeeds with `a` or is ambiguous — both are valid behaviors
        match result {
            Ok(id) => assert_eq!(id, a),
            Err(PrefixError::Ambiguous { .. }) => {} // fine, unlikely but possible
