...
//!
//! Slice #3 ([`audio`], [`prefetch`], 2026-07-18) is the first LIVE wiring:
//! [`thread::DjPlugin`] is now registered in `main.rs` (replacing the
//! deleted `audio::AudioOutPlugin`), and the DJ thread dispatches every
//! `audio/*`, [`kaijutsu_audio::CLIP_MIME`], and
//! [`kaijutsu_audio::PREPARE_MIME`] `RenderCue` for real — ported from the
//! deleted `audio.rs` (`play_render_cues`/`drain_prefetch_results`/
//! `CasPrefetch`) with one structural change: [`prefetch::CasPrefetch`]'s
//! outcome channel is a `tokio::mpsc` unbounded pair instead of
//! `crossbeam_channel`, so `thread::run_loop`'s `select!` gets a native async
//! arm for it instead of a per-frame drain. `text/vnd.abc`, clicks, and
//! `BeatSync` sinks stay on the old Bevy-side path (`midi.rs`,
