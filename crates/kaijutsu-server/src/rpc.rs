...
/// dedicated thread + LocalSet supports the reconcile path, which
...
pub(crate) use kaijutsu_kernel::runtime::synthesis::BlockStoreSource;
...
impl kernel::Server for KernelImpl {
    ...
    fn complete_block(
        ...
        if is_error != (status == Status::Error) {
            results.get().set_success(false);
            results.get().set_error(format!(
                "completeBlock: isError={is_error} contradicts status={}; \
                 Status::Error is the error state for a completed block",
                status.as_str()
            ));
            return Promise::ok(());
        }
