...
impl KernelHandle {
    ...
    /// Read a key from the kernel KV. Returns `None` if absent, deleted, or
    /// advisory-expired.
    #[tracing::instrument(skip(self), name = "rpc_client.kv_get")]
    pub async fn kv_get(&self, key: &str) -> Result<Option<String>, RpcError> {
        let mut request = self.kernel.kv_get_request();
        request.get().set_key(key);
        let response = request.send().promise.await?;
        let result = response.get()?;
        if result.get_found() {
            Ok(Some(result.get_value()?.to_string()?))
        } else {
            Ok(None)
        }
    }

    /// Set a key in the kernel KV. `expires_at` is an advisory absolute expiry
    /// (writer-clock ms). Surfaces the server's in-band error (value cap,
    /// persistence fault) as `RpcError::ServerError`.
    #[tracing::instrument(skip(self, value), name = "rpc_client.kv_set")]
    pub async fn kv_set(
        &self,
        key: &str,
        value: &str,
        ...
        let mut request = self.kernel.kv_set_request();
        request.get().set_key(key);
        request.get().set_value(value);
        request.get().set_has_expires_at(expires_at.is_some());
        request.get().set_expires_at(expires_at.unwrap_or(0));
        let response = request.send().promise.await?;
        let result = response.get()?;
        if !result.get_success() {
            let error = result.get_error()?.to_str()?;
    ...
    /// Delete a key. Returns whether a live value existed.
    #[tracing::instrument(skip(self), name = "rpc_client.kv_delete")]
    pub async fn kv_delete(&self, key: &str) -> Result<bool, RpcError> {
        let mut request = self.kernel.kv_delete_request();
        request.get().set_key(key);
        let response = request.send().promise.await?;
        Ok(response.get()?.get_existed())
    }

    /// List KV keys, optionally filtered by prefix.
    #[tracing::instrument(skip(self), name = "rpc_client.kv_keys")]
    pub async fn kv_keys(&self, prefix: Option<&str>) -> Result<Vec<String>, RpcError> {
        let mut request = self.kernel.kv_keys_request();
        request.get().set_has_prefix(prefix.is_some());
        request.get().set_prefix(prefix.unwrap_or(""));
        let response = request.send().promise.await?;
        let keys = response.get()?.get_keys()?;
        let mut out = Vec::with_capacity(keys.len() as usize);
        for k in keys.iter() {
