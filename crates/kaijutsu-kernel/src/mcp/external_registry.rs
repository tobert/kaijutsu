//! Reconciles `/config/kernel/mcp.toml`, an ordinary host file
//! (docs/config-namespace.md), with
...
/// The real entry point: read `/config/kernel/mcp.toml` through the VFS (an
/// ordinary host file), then reconcile.
///
