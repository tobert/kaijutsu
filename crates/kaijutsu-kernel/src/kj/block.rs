...
impl KjDispatcher {
    ...
    fn block_create(&self, args: BlockCreateArgs, caller: &KjCaller) -> KjResult {
        ...
        let declared_content_type = ContentType::from_mime(&content_type);

        // Raster image content is a CAS-hash reference, never inline bytes
        // (see `BlockCreateArgs::content_type`'s doc). A caller can declare
        // any `--content-type` and point `--content` at any string; sniff
        // the referenced object's actual bytes and refuse loudly rather
        // than label something that isn't a matching image as
        // `ContentType::Image`.
        if declared_content_type == ContentType::Image {
            if let Err(e) = self.validate_image_block_content(&content_type, &content_str) {
                return KjResult::Err(format!("kj block create: {e}"));
            }
        }
    ...
    /// Refuse `block create --content-type <image mime>` unless `content`
    /// is a CAS hash whose referenced bytes actually sniff as that image
    /// format. Header sniffing only — reads a small prefix off disk via
    /// `ContentStore::path`, never the full object.
    fn validate_image_block_content(
        &self,
        content_type: &str,
        content: &str,
    ) -> Result<(), String> {
        let hash = content.parse::<kaijutsu_cas::ContentHash>().map_err(|_| {
            format!(
                "--content-type {content_type} requires --content to be a CAS hash \
                 (32 hex chars); '{content}' is not one"
            )
        })?;
        let cas = self.kernel().cas();
        ...
        let n = {
            ...
            std::fs::File::open(&obj_path)
                ...
                .map_err(|e| format!("reading CAS object {hash}: {e}"))?
        };
        super::cas::validate_image_bytes(content_type, &header[..n])
            .map_err(|e| format!("{e} (CAS hash {hash})"))
    }
}
...
mod tests {
    ...
    // ── block create: image content-type must match the CAS bytes ──────
    // A 1x1 real PNG
    // and a 1x1 real JPEG, so these tests exercise the same magic-number
    // path a real asset would.
    const TEST_REAL_PNG: &[u8] = &[
    ...
    /// Regression (a): a CAS hash whose bytes are a real PNG, declared as
    /// `image/jpeg`, must be refused with both types named.
    #[tokio::test]
    async fn block_create_refuses_png_bytes_declared_as_jpeg() {
        let d = test_dispatcher().await;
        let ctx = register_context_with_doc(&d, Some("img"), PrincipalId::new());
        let caller = caller_with_context(ctx);
        let hash = d.kernel().cas().store(TEST_REAL_PNG, "image/png").expect("cas store");

        let result = d.dispatch(&[
            s("block"), s("create"), s("--role"), s("asset"), s("--kind"), s("file"),
            s("--content-type"), s("image/jpeg"), s("--content"), hash.to_string(),
        ], &caller).await;
        assert!(!result.is_ok(), "PNG bytes declared as jpeg must be refused");
        assert!(result.message().contains("image/jpeg"), "msg: {}", result.message());
        assert!(result.message().contains("image/png"), "msg: {}", result.message());
        assert!(d.block_store().block_snapshots(ctx).unwrap().is_empty(), "no block must be created");
    }

    /// Regression (b): a CAS hash whose bytes are plain text, declared as
    /// `image/png`, must be refused.
    #[tokio::test]
    async fn block_create_refuses_text_bytes_declared_as_png() {
        let d = test_dispatcher().await;
        let ctx = register_context_with_doc(&d, Some("img"), PrincipalId::new());
        let caller = caller_with_context(ctx);
        let hash = d
            .kernel()
            .cas()
            .store(b"just some plain text, not an image", "image/png")
            .expect("cas store");

        let result = d.dispatch(&[
            s("block"), s("create"), s("--role"), s("asset"), s("--kind"), s("file"),
            s("--content-type"), s("image/png"), s("--content"), hash.to_string(),
        ], &caller).await;
        assert!(!result.is_ok(), "text bytes declared as png must be refused");
        assert!(result.message().contains("image/png"), "msg: {}", result.message());
        assert!(d.block_store().block_snapshots(ctx).unwrap().is_empty(), "no block must be created");
    }
    ...
    /// accepted.
    #[tokio::test]
    async fn block_create_accepts_genuine_png_and_jpeg() {
        let d = test_dispatcher().await;
        let ctx = register_context_with_doc(&d, Some("img"), PrincipalId::new());
        let caller = caller_with_context(ctx);

        let png_hash = d.kernel().cas().store(TEST_REAL_PNG, "image/png").expect("cas store");
        let result = d.dispatch(&[
            s("block"), s("create"), s("--role"), s("asset"), s("--kind"), s("file"),
            s("--content-type"), s("image/png"), s("--content"), png_hash.to_string(),
        ], &caller).await;
        assert!(result.is_ok(), "genuine PNG create failed: {}", result.message());

        let jpeg_hash = d.kernel().cas().store(TEST_REAL_JPEG, "image/jpeg").expect("cas store");
        let result = d.dispatch(&[
            s("block"), s("create"), s("--role"), s("asset"), s("--kind"), s("file"),
            s("--content-type"), s("image/jpeg"), s("--content"), jpeg_hash.to_string(),
        ], &caller).await;
        assert!(result.is_ok(), "genuine JPEG create failed: {}", result.message());

        let snaps = d.block_store().block_snapshots(ctx).unwrap();
        assert_eq!(snaps.len(), 2, "both genuine images must have created blocks");
        assert!(snaps.iter().all(|b| b.content_type == ContentType::Image));
    }

    /// A caller-declared image type whose `--content` isn't even a CAS
    /// hash must be refused before touching the CAS.
    #[tokio::test]
    async fn block_create_refuses_non_hash_content_for_image_type() {
        let d = test_dispatcher().await;
        let ctx = register_context_with_doc(&d, Some("img"), PrincipalId::new());
        let caller = caller_with_context(ctx);
        ...
        let result = d.dispatch(&[
            s("block"), s("create"), s("--role"), s("asset"), s("--kind"), s("file"),
            s("--content-type"), s("image/png"), s("--content"), s("not-a-cas-hash"),
        ], &caller).await;
        assert!(!result.is_ok(), "non-hash content for an image type must be refused");
        assert!(d.block_store().block_snapshots(ctx).unwrap().is_empty(), "no block must be created");
    }
