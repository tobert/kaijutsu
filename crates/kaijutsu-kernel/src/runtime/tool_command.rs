...
pub(crate) struct ToolCommand {
    /// Reserved by the caller before `admission` was minted and before any
    /// gate ask (`docs/resource-admission.md`, rule 1).
    pub slot: crate::runtime::RuntimeSlot,
...
mod setup_tests {
    ...
    async fn waiting_model_pair_retains_its_execution_owner_in_the_same_acceptance() {
        ...
        for fault in [None, Some("receipt"), Some("link"), Some("context"), Some("actor"), Some("plain")] {
            ...
            let mut events = kernel.block_flows().subscribe("block.*");
            let accepted = kernel.blocks().settle_tool_result_as(context, &command, &output, "waiting for approval", Default::default(),
